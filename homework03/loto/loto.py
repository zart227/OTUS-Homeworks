...
class Card:
    ...
    def generate_card(self):
        ...
        card = [numbers[0:5], numbers[5:10], numbers[10:15]]
        for row in card:
            row.sort()
    ...
    def __str__(self):
        return "\n".join(
            " ".join(str(num).rjust(2) for num in row) for row in self.grid
        )
...
class Game:
    ...
    def play(self):
        ...
        while True:
            if not self.barrels:
                print("Игра окончена. Все бочонки были использованы.")
                return
            ...
            for player in self.players:
                ...
                if player.is_computer:
                    ...
                    while True:
                        action = input(
                            f"{player.name}, зачеркнуть цифру? (y/n): "
                        ).lower()
                    ...
                    if action == "y":
                        ...
                        if mark:
                            ...
                            print(
                                f"Цифры {called_number} нет на вашей карточке. Вы проиграли!"
                            )
                            return
                    else:
                        mark = False
...
if __name__ == "__main__":
    ...
    for i in range(num_players):
        ...
        is_computer = (
            input(f"Является ли игрок {name} компьютером? (y/n): ").lower() == "y"
        )
        players.append(Player(name, is_computer))
